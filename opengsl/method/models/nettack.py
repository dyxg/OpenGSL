...
from opengsl.data.preprocess import normalize
...
class BaseAttack(Module):
    ...
    def save_adj(self, root=r'/tmp/', name='mod_adj'):
        ...
        if type(modified_adj) is torch.Tensor:
            modified_adj = sparse_tensor_to_scipy_sparse(modified_adj)
    ...
    def save_features(self, root=r'/tmp/', name='mod_features'):
        ...
        if type(modified_features) is torch.Tensor:
            modified_features = sparse_tensor_to_scipy_sparse(modified_features)
...
class Nettack(BaseAttack):
    ...
    def get_linearized_weight(self):
        ...
        W = surrogate.convs[0].mlp[0].weight.transpose(0,1) @ surrogate.convs[1].mlp[0].weight.transpose(0,1)
    ...
    def attack(self, features, adj, labels, target_node, n_perturbations, direct=True, n_influencers= 0, ll_cutoff=0.004, verbose=True, **kwargs):
        ...
        if type(adj) is torch.Tensor:
            self.ori_adj = to_scipy(adj).tolil()
            self.modified_adj = to_scipy(adj).tolil()
            self.ori_features = to_scipy(features).tolil()
            self.modified_features = to_scipy(features).tolil()
        else:
        ...
        self.adj_norm = sparse_tensor_to_scipy_sparse(normalize(scipy_sparse_to_sparse_tensor(self.modified_adj).to_dense()).to_sparse())
        ...
        for _ in range(n_perturbations):
            ...
            if change_structure:
                ...
                self.adj_norm = (self.modified_adj)
